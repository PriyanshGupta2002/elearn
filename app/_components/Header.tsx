  ...
  return (
    <div className="p-4 max-w-7xl flex justify-between items-center w-full">
      <div className="flex gap-2 items-center">
        ...
        <h2 className="font-pixelify-sans text-3xl font-bold">CodeBox</h2>
      </div>
      {/* Navbar */}
      <NavigationMenu>
            ...
            <NavigationMenuContent>
              <ul className="grid grid-cols-2 gap-2 sm:w-[400px] md:w-[500px] lg:w-[600px]">
                {courses.map((course, index) => (
                  <li
                    key={index}
                    className="hover:bg-accent p-2 rounded-xl cursor-pointer h-fit"
                  >
      ...
      {/* Signup Button */}
      <Button className="font-pixelify-sans" variant={"pixel"}>
        Signup
      </Button>
    </div>
  );
